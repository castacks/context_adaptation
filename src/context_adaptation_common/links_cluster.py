...
class Subcluster:
    ...
    def __init__(self, initial_vector: np.ndarray, store_vectors: bool = False,n_vectors = None):
...
class LinksCluster:
    ...
    def __init__(self,
        ...
        if initial_centroids is not None:
            for i in range(initial_centroids.shape[0]):
                ...
                self.clusters.append([Subcluster(centroid, store_vectors=self.store_vectors,n_vectors = n_vectors)])

    def predict(self, new_vector, return_probs=False):
        ...
        for cl_idx, cl in enumerate(self.clusters):
            ...
            for sc_idx, sc in enumerate(cl):
                if self.dist_metric == 'cosine':
                    ...
                    # print(cossim)
                    # print(new_vector.shape, sc.centroid.T.shape)
                    # cossim = 1 + ((new_vector @ sc.centroid) / (np.linalg.norm(new_vector)*np.linalg.norm(sc.centroid)))
                    # print(cossim)
                elif self.dist_metric == 'euclidean':
                    cossim = (self.pair_similarity_maximum - sqeuclidean(new_vector, sc.centroid))/self.pair_similarity_maximum

                # print(cossim)

                if cossim > best_cluster_sim:
                    best_cluster_sim = cossim
            ...
            # print(best_cluster_sim)
            probs.append(best_cluster_sim)
        # print('---')
        if best_similarity >= self.subcluster_similarity_threshold:  # eq. (20)
            ...
            if self.dist_metric == 'cosine':
                cossim = 1.0 - cosine(new_subcluster.centroid, best_subcluster.centroid)
                # cossim = 1 + ((new_subcluster.centroid @ best_subcluster.centroid) / (np.linalg.norm(new_subcluster.centroid)*np.linalg.norm(best_subcluster.centroid)))
            elif self.dist_metric == 'euclidean':
                cossim = (self.pair_similarity_maximum - sqeuclidean(new_subcluster.centroid, best_subcluster.centroid))/self.pair_similarity_maximum
    ...
    def update_edge(self, sc1: Subcluster, sc2: Subcluster):
        ...
        # cossim = 1 + ((sc1.centroid @ sc2.centroid) / (np.linalg.norm(sc1.centroid)*np.linalg.norm(sc2.centroid)))
    ...
    def update_cluster(self, cl_idx: int, sc_idx: int):
        ...
        for connected_sc in connected_scs:
            ...
            # cossim = 1 + ((updated_sc.centroid @ connected_sc.centroid) / (np.linalg.norm(updated_sc.centroid)*np.linalg.norm(connected_sc.centroid)))
                ...
                for cluster_sc in self.clusters[cl_idx]:
                    if cluster_sc != severed_sc:
                        cossim = 1.0 - cosine(cluster_sc.centroid,
                                              severed_sc.centroid)
                        # cossim = 1 + ((cluster_sc.centroid @ severed_sc.centroid) / (np.linalg.norm(cluster_sc.centroid)*np.linalg.norm(severed_sc.centroid)))
