...
for i,lap in enumerate(laps):
    ...
    ax[0,i].imshow(np.transpose(map,(1,2,0)))
    ax[0,i].scatter(fig8_c,fig8_r,s=40,c='w')
    im = ax[i].scatter(col,row,c=gps_vels,cmap='jet',vmin=0.0,vmax=6.0,s=1.6)

    ax[0,i].set_xlim([SCALE*200/.2, SCALE*250/.2])
    ax[0,i].set_ylim([SCALE*180/.2, SCALE*90/.2])

    ax[0,i].set_title("Lap " + str(i+1))
    ax[0,i].axis("off")
