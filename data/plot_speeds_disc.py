...
def unique_disc(input, thresh = .8):
    points = input.copy()
    pts = []
    while points.shape[0] > 1:
        pt = points[0]
        # print(pt.shape)
        # print((pt[:2] - points[:,:2]).shape)
        dists = np.linalg.norm(pt[:2] - points[:100,:2],axis=1)
        # print(dists)
        # print(dists.shape)
        close_points = points[:100][dists < thresh]
        sel_id = np.argmax(close_points[:,3])
        pts.append(close_points[sel_id])
        points = np.vstack([points[:100][dists > thresh], points[100:]])
        # print(points.shape)
...
for i,lap in enumerate(laps):
    ...
    cur_gps = cur_gps[cur_gps[:,3] > .2]

    cur_gps = unique_disc(cur_gps)
