...
class Grid:
    ...
    def __init__(
        ...
        self.bounding_box = Cube(side_length=8, color=GRAY, fill_opacity=0.05)
        self.scene.add(self.bounding_box)

        self.all_data, self.all_tsne = init_data()

        buff_size = 200
        t = buff_size
        self.buff_intelligent = all_data[:buff_size].copy()
        self.intelligent_tsne = all_tsne[:buff_size].copy()
        self.buff_classes = np.argmin(buff_intelligent[:,:8],axis=1)

        self.buff_FIFO = all_data[:buff_size].copy()
        self.FIFO_tsne = all_tsne[:buff_size].copy()
        self.buff_toi = np.arange(buff_FIFO.shape[0])
    ...
    def fadeIn(self):
        self.scene.play(
            ...
            *[FadeIn(self.grid[index][0]) for index in self.grid],
        )
