...
SMAP = matplotlib.cm.get_cmap('RdYlGn')
...
class Context_Clusterer(object):
    def __init__(self,cost_topic, odom_topic , gridmap_topic, costmap_topic, speedmap_in_topic, speedmap_out_topic, vel_pub_topic, config):
        ...
        self.speedmap_pub = rospy.Publisher(speedmap_out_topic,GridMap,queue_size=2)
        ...
        self.speed_map_cvt = GridMapConvert(channels=['speedmap'], size=[1, 1])
        ...
        rospy.Subscriber(speedmap_in_topic, GridMap, self.handle_speedmap, queue_size=1)
    ...
    def run_map(self, event):
        ...
        with self._lock:
            if self.dino_map is not None:
                ...
                #HACK assuming speedmap and costmap are the same size (they should be?)
                if self.speed_map is not None:
                    s_info = self.speed_map.info
                    s_nx = int(s_info.length_x / s_info.resolution)
                    s_ny = int(s_info.length_y / s_info.resolution)
                    self.speed_map_cvt.size = [s_nx, s_ny]
                    speedmap = self.speed_map_cvt.ros_to_numpy(self.speed_map)['data'][0]
                    speedmap_header = self.speed_map.info.header

                    # print("TIMESTAMPS ", speedmap_header.stamp, self.dino_map.info.header.stamp)
                    # print("TIMESTAMPS ", (speedmap_header.stamp - self.dino_map.info.header.stamp)/1e9)
                    print("DIMS", nx, s_nx)
                else:
                    speedmap = None
        ...
        #HACK
        var_map = 1-(costmap_var - .05)

        costmap_msg = self.costmap_to_gridmap(costmap, info)
        self.costmap_pub.publish(costmap_msg)

        if speedmap is not None:
            speedmap *= var_map
            speedmap_msg = self.speedmap_to_gridmap(speedmap, info)
    ...
    def speedmap_to_gridmap(self, costmap, info, costmap_layer='speedmap'):
        """
        convert costmap into gridmap msg
...
def main():
    ...
    speedmap_in_topic = rospy.get_param("~speedmap_in_topic")
    speedmap_out_topic = rospy.get_param("~speedmap_out_topic")
    ...
    node = Context_Clusterer(cost_topic, odom_topic , gridmap_topic, costmap_topic,  speedmap_in_topic, speedmap_out_topic, vel_pub_topic, config_dict)
    rate = rospy.Rate(10)
