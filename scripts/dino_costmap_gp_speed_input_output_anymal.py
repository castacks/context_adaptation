...
class Context_Clusterer(object):
    def __init__(self,cost_topic, odom_topic , gridmap_topic, costmap_topic, vel_pub_topic, config):
        ...
        #wall - [23.136944 23.72807  24.30309  24.238636 25.431227 25.023436 24.36344123.660805]
        #dead tree - [23.475155 24.631695 24.692223 23.26986  25.572502 24.595654 21.708942, 24.279928]

        avoid_data[:,-1] = 1.0
        for i in range(num_insert):
            avoid_data[i,-2] = i*spread
            avoid_data[i,:self.VLAD_CLUSTS] = torch.Tensor([23.136944, 23.72807,  24.30309,  24.238636,
             25.431227, 25.023436, 24.363441, 23.660805]).cuda() / self.residual_max
        ...
        for i in range(num_insert):
            avoid_data[i,-2] = i*spread
            avoid_data[i,:self.VLAD_CLUSTS] = torch.Tensor([23.475155, 24.631695 ,24.692223, 23.26986,
              25.572502, 24.595654, 21.708942, 24.279928]).cuda() / self.residual_max
            # avoid_data[i,-1] += torch.randint(-1000,1001,[1])[0] * .0001
            # avoid_data[i,:self.VLAD_CLUSTS] += torch.randint(-1000,1001,[self.VLAD_CLUSTS]).cuda() * .0001
        ...
        self.gp_params = torch.load(os.path.join(assets_dir, 'gp_params', 'gp_params_anymal'))
        ...
        rospy.Subscriber(odom_topic, PoseWithCovarianceStamped, self.handle_odom, queue_size=1)
        rospy.Subscriber('/motion_reference/command_twist', TwistStamped, self.handle_twist, queue_size=1) #use this to get velocity for now
    ...
    def handle_cost(self, msg):
        ...
        self.cost = 1.0 - self.cost
        # print("COST - ", self.cost)
        self.rough_history = self.rough_history + (msg.data - self.rough_history) * .5
        # print('cost', self.cost)
    ...
    def run_map(self, event):
        ...
        # np.save('/home/physics_atv/physics_atv_ws/gridmap_data_anymal', gridmap['data'])
        ...
        if self.hz_counter % 20 == 0:# and self.gp_params is not None:
            ...
            torch.save(self.gp_params, '/home/physics_atv/physics_atv_ws/src/learning/context_adaptation/assets/gp_params/gp_params_anymal')
            ...
            # print("shape ", .shape)
            # np.save("support_data_turnpike", np.array(self.support_data))
    ...
    def costmap_to_gridmap(self, costmap, info, costmap_layer='costmap', variance = None, norm_factor = None):
        ...
        if variance is not None:
            layer_data = np.zeros_like(costmap) + self.odom_msg.pose.pose.position.z - .5 - variance*5
        else:
            layer_data = np.zeros_like(costmap) + self.odom_msg.pose.pose.position.z - .5 #+ costmap
