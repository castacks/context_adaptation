...
class Context_Clusterer(object):
    def __init__(self,cost_topic, odom_topic , gridmap_topic, costmap_topic, vel_pub_topic, config):
        ...
        rospy.Subscriber('/wheelchair_speed', Float32, self.handle_vel, queue_size=1)
    ...
    def handle_cmd(self, msg):
        ...
        self.desired_yawrate = msg.angular.z
    ...
    def run_map(self, event):
        ...
        if (self.velocity > .05) and self.hz_counter % self.buffer_update_freq == 0:
            # print("UPDATING+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
            ...
            input_sample = torch.cat((input_sample, torch.Tensor([np.random.normal()*.0001]),torch.Tensor([self.cost]))).cuda()
        ...
        with torch.no_grad():
            if self.buffer_idx < 5 + self.num_insert: #we need at least a couple samples apart from single label
                ...
                vel_append = torch.ones(input.shape[0],1) * np.random.normal()*.0001
    ...
    def costmap_to_gridmap(self, costmap, info, costmap_layer='costmap', variance = None, norm_factor = None):
        ...
        if variance is not None:
            layer_data = np.zeros_like(costmap) + self.odom_msg.pose.pose.position.z - .3 - variance*5
        else:
            layer_data = np.zeros_like(costmap) + self.odom_msg.pose.pose.position.z - .3 #+ costmap
